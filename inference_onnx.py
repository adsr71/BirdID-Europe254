...
# onnx
modelOnnxPath = 'onnx/BirdID-Europe-254-Bc24-EffNetB2.onnx'
...
#         print('ort.get_available_providers()', available_providers) # ['TensorrtExecutionProvider', 'CUDAExecutionProvider', 'CPUExecutionProvider']
...
#     ort_session = ort.InferenceSession(modelOnnxPath, providers=providers)
...
def loadOnnxModel():
    ...
    # Set number of threads to use for parallel execution.
    sess_options.intra_op_num_threads = cfg.nCpuWorkers
    sess_options.inter_op_num_threads = cfg.nCpuWorkers
    ...
    ort_session = ort.InferenceSession(modelOnnxPath, sess_options=sess_options, providers=providers)
...
class AudioDatasetOnnx:

    def __init__(self, audioDicts):
    ...
    def __getitem__(self, segmentIx):
        ...
        specImage = getSpecSegment(
            self.audioDicts, fileIx, channelIx, startSample, endSample, self.segmentDurationInSamples
        )

        # Convert spectrogram to 3-channel image (similar to AudioDataset)
        h, w = specImage.shape
        c = 3
        ...
        threeChannelSpec = threeChannelSpec.astype(np.float32)
        # Normalize to [0, 1]
        threeChannelSpec /= 255.0

        # Normalize using mean and std
        threeChannelSpec -= cfg.imageMean
        threeChannelSpec /= cfg.imageStd

        # h,w,c --> c,h,w
        threeChannelSpec = np.transpose(threeChannelSpec, (2, 0, 1))

        return threeChannelSpec
...
def processFilesOnnx(ort_session, audioPath):
    ...
    for ix in range(0, len(paths), cfg.batchSizeFiles):
        ...
        if None in audioDicts:
            audioDicts = [x for x in audioDicts if x is not None]

        # Prepare audio dataset for inference using AudioDatasetOnnx
        audioDataSetObj = AudioDatasetOnnx(audioDicts)
        ...
        outputs = []
        for i in range(0, len(audioDataSetObj), cfg.batchSizeInference):
            
            batch = [audioDataSetObj[j] for j in range(i, min(i + cfg.batchSizeInference, len(audioDataSetObj)))]
            # # Use multiprocessing to create batches --> not working correctly !!!
            # batch_indices = range(i, min(i + cfg.batchSizeInference, len(audioDataSetObj)))
            # with Pool(cfg.nCpuWorkers) as p:
            #     batch = p.map(audioDataSetObj.__getitem__, batch_indices)
            
            batch = np.stack(batch)  # Convert batch to NumPy array
...
# worker_audio_dataset = None

# def init_worker(dataset):
#     """Initializer for multiprocessing worker.
#     This makes the dataset object available to the worker process.
#     """
#     global worker_audio_dataset
#     worker_audio_dataset = dataset

# def process_segment(segmentIx):
#     """Worker function to process a single segment using the global dataset."""
#     # The worker_audio_dataset is a global variable in this process
...
#         # Perform inference using ONNX with a producer-consumer pattern
#         outputs = []
#         batch = []
        
#         with Pool(processes=cfg.nCpuWorkers, initializer=init_worker, initargs=(audioDataSetObj,)) as p:
#             # p.imap_unordered processes segments in parallel and yields results as they complete.
#             # This allows the main process to run inference on the first batch while 
#             # subsequent batches are still being processed by the workers.
#             results_iterator = p.imap_unordered(process_segment, range(len(audioDataSetObj)))

#             for segment_result in results_iterator:
#                 batch.append(segment_result)

#                 # When a full batch is ready, run inference
#                 if len(batch) == cfg.batchSizeInference:
...
#         # Process the last, possibly incomplete, batch
#         if batch:
#             batch_np = np.stack(batch)
#             ort_inputs = {ort_session.get_inputs()[0].name: batch_np}
#             ort_outs = ort_session.run(None, ort_inputs)
